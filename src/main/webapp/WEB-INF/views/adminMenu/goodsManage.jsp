		...
		<c:if test="${listcheck != 'empty'}">
			...
			<table class="goods_table">
				...
				<c:forEach items="${list}" var="list">
					<tr  class="neon_effect">

						<td >${list.shipId}</td>

						<td><a class="move" href='<c:out value="${list.shipId}"/>'><c:out
									value="${list.shipName}"></c:out></a></td>
		...
		<ul class="pageMaker">
			...
			<c:forEach begin="${pageMaker.pageStart}" end="${pageMaker.pageEnd}"
				...
				<li class="pageMaker_btn ${pageMaker.cri.pageNum == num ? "active":""}">
					<a class="pageMoveBtn" href="${num}">${num}</a>
				</li>
			</c:forEach>
		...
		$(".move")
				.on(
						...
						function(e) {
							e.preventDefault();
							moveForm
									.append("<input type='hidden' name='shipId' value='"
											+ $(this).attr("href") + "'>");
